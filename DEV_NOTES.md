...
- 配置增加`playbackDeliveryMode=auto|proxy`并默认使用`auto`；旧配置自动补齐，不修改SQLite user_version 5、JSON schema 13或迁移包schema 3。
- 受保护播放接口先完成原有登录、关系、远端证明和路径校验。AList返回外部公共HTTPS 302时直接交给浏览器，HTTP、相对、同AList、本机、私网及畸形跳转继续由BFB代理。
- 302响应禁止缓存和发送来源页，不记录签名URL；浏览器非HEVC直连失败时只带`delivery=proxy`回退一次并恢复当前位置，手动重试重新获取直链。
- 阿里云部署只读探测确认当前天翼驱动返回外部HTTPS 302，目标不带AList凭据且支持`206 Range`；探测仅请求1字节，未修改服务器数据。
- 完整回归共196项：195项通过、1项因本机缺少aria2跳过、0项失败；播放、配置与收藏夹详情专项36项全部通过。
- 脱敏浏览器夹具确认302目标直接返回`206 Range`且不携带BFB页面来源；模拟直链失效后仅回退一次代理并恢复可播放状态。1280x720、390x844和844x390均无横向溢出、控件遮挡或控制台错误。

## 未发布手机沉浸竖屏

- 宽度不超过720px且处于竖屏方向时默认使用全视口播放舞台；偏好保存在现有`bfb-playback-v1`本地存储中，旧记录缺少字段时默认开启，不增加服务端配置、API或schema。
- 上下滑动复用现有`P1 -> Pn -> 下一视频`顺序，72px阈值并要求纵向位移占优；按钮、进度条、设置和输入控件不会触发切换，同一次手势完成前禁止重复换集。
- 队列和搜索改为沉浸模式底部抽屉，关闭抽屉及切换视频不滚动外层视口；设备转为横屏时自动回到原播放器布局，转回竖屏后按浏览器偏好恢复，全程不调用方向锁定API且媒体保持`object-fit: contain`。
- 播放器会话销毁时同时清除队列视图标记，关闭后重开会重新渲染当前50项，不再出现右侧队列空白。
