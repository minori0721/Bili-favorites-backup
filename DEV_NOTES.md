...
- 修复启动恢复与缓存快照的唤醒竞态：缓存占用扫描完成后会主动重新派发到期持久任务，不再需要保存配置或手动同步才能启动；`stop → start` 也会立即恢复已经到期的任务。
- 收藏夹轮询定时器与持久任务唤醒器分离，重启轮询不会清除未来任务的 `not_before` 唤醒；租约心跳不会再被任务派发误停。缓存背压期间到期任务使用最小 1 秒唤醒间隔，避免零延迟空转。
...
- 调度极端场景：覆盖缓存扫描失败后自动恢复、扫描中停止、多个缓存回调并发、完整 `stop → start`、连续重复 `start`、缓存满背压和租约心跳保持；并发回调只会事务领取一次，停止后不会异步误派发。
...
- 当前自动化结果（2026-07-12 复测）：`npm test` 共 86 项，85 通过、0 失败；默认环境仅因本机未安装 aria2 跳过 1 项。新增的 7 项调度回归全部通过，`npm run build` 与 `git diff --check` 通过。此前已使用官方 aria2 1.37.0 单独实跑 Range 断线续传测试并通过。
