...
- 分支：`dev`
- 基准版本：`2.3.3`
- 当前 dev 变更：上传可靠性、文件名兼容、失败隔离补传、启动恢复限流、熔断与下载背压修复，尚未合并到 `main`。
...
- 状态与恢复：schema 8→9、关系级部分失败、多目标完成后清理、批量保存、全局补传预算、补传优先级和单文件确定性失败解锁。
...
- 当前自动化结果：`npm test` 共 31 项，全部通过；`npm run build` 与 `git diff --check` 通过。
