...
- 修复“手动触发同步”接口在任务进行中仍返回成功的问题：现在会返回 `409` 并提示已有同步正在运行，避免前端误判已触发。
- 修复远端核验全量模式类型问题：`listVideosForRemoteVerify` 现支持不限量拉取，便于完整远端对账。
- 修复 Hi-Res / 杜比参数“仅启用 app 模式未参与清晰度优先级”的问题：现在会将勾选项并入 `--dfn-priority`，确保参数对下载结果实际生效。
- 修复远端状态回查不充分问题：将 `uploaded/verified` 且存在 `remotePath` 的历史视频纳入核验范围，不再仅依赖 `remoteFiles`，避免 AList 手工删除后长期误判“仍已备份”。
- 修复 AList 删除后缺少补偿触发问题：当远端目录为空或文件缺失时，状态会进入缺失计数并自动尝试回补入队（视频仍可访问时）。
- 修复自动同步“无新增”无日志问题：每轮同步结束都会写入摘要日志，明确显示“无新增视频”及远端核验数量、缺失数量与耗时。

### 优化

- 增强远端核验吞吐：手动同步与“本轮无新增”场景下提高远端核验预算，加快全量回查覆盖速度。
- 新增“全量对账并回补”触发链路：后端新增 `/api/sync/reconcile`，前端新增按钮 `Full Reconcile`，用于在运行中按需执行一次全量远端核验与缺失回补入队。
- 简化运维排查：新增每轮同步汇总原始日志字段，包含新增/入队/核验/缺失/回补/错误和耗时指标。
- 降低误判：远端缺失会在同轮进行二次确认，确认仍缺失后才推进到可回补状态，减少瞬时网络抖动带来的误触发。
