...
test('quality projection preserves grouping, retry labels and lease-derived running state without writes',()=>{
  const base:PersistentJobRecord={id:'job',kind:'quality_download' as const,dedupeKey:'fixture',bvid:'BV',userId:'user',mediaId:1,status:'running' as const,priority:0,payload:{artifactKey:'artifact',videoTitle:'fixture',qualityStageLabel:'下载新版 · stale'},attempts:0,maxAttempts:3,notBefore:0,createdAt:100,updatedAt:200};
  const jobs=Object.freeze([Object.freeze(base),Object.freeze({...base,id:'cleanup',kind:'quality_cleanup' as const,status:'retry_wait' as const})]);
