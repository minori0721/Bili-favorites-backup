...
test('queue status preserves zero counts and both persisted numeric and ISO schedule timestamps',()=>{
  const status=parseQueueStatus({scheduler:{status:'idle',nextRunAt:100,queuedActions:['同步']},recovery:{pendingUploads:0},chargingAccess:{pending:1,nextCheckAt:'2026-09-08T00:00:00Z'},localCache:{usedBytes:0,limitBytes:10,paused:false}});
