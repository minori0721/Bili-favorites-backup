...
test('login boundary accepts QR images and rejects malformed sessions and status responses', () => {
  ...
  assert.equal(parseLoginStatus({status:'completed' as const}).status, 'completed');
  for (const value of [null, {}, {status:'unknown' as const}, {status:'error' as const,message:{}}]) assert.throws(() => parseLoginStatus(value));
});
