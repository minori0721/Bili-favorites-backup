...
- **归档播放**：在收藏夹详情中直接播放远端已验证的MP4、M4V和WebM；优先使用网盘临时直链并自动回退BFB代理，支持分P、连续播放、进度恢复、手机沉浸竖屏、全屏和画中画。
...
`v2.4.4`继续使用SQLite `user_version 5`。schema 1至4首次升级前会在`data/backups`生成一致性数据库备份和SHA256摘要；仍使用schema 4或更早版本的旧镜像不能直接打开schema 5数据库，回滚时应恢复该备份或使用兼容JSON导出。

从`v2.4.3`及更早版本更新后需要重新登录一次；之后可在登录页选择固定保持30天。业务SQLite、JSON兼容状态和迁移包schema在本次升级中均不变化。
...
归档播放器先由BFB校验登录和文件归属；AList提供合格的外部HTTPS临时直链时使用302让浏览器直连网盘，否则自动回退BFB流式代理。直连能减少BFB服务器流量，但临时签名地址会在浏览器网络面板中可见；AList地址和凭据不会下发。播放器不执行转码，HEVC等编码能否播放取决于浏览器和设备的原生解码能力。
