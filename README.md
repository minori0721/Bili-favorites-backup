...
- 按“账号 + 收藏夹 + BV号”记录备份状态，同一视频在多个收藏夹中会分别对账和补传。
- 已失效视频会保留历史标题、UP 主、简介和封面快照；如果是当前账号本人可见的自投稿，收藏夹详情会尝试回填真实元信息，不再只显示“已失效视频”。
- 新增数据迁移包，可导出/导入配置、账号、备份状态、下架记录、日志和封面缓存；导入前会自动备份当前 `data`。
- 优化 Docker 跨挂载封面缓存写入，并修复长标题视频列表中右侧状态被挤出的问题。
