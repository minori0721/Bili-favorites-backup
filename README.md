...
- AList 上传改为发送准确文件大小并校验远端结果；上传异常时保留本地文件为“待补传”，暂停新下载并在后端恢复后自动续传。
...
- **🧯 上传故障保护**：AList 认证、驱动或网络持续异常时自动暂停新下载；已下载文件不会因上传失败被删除，恢复后优先补传。
...
- **失败任务优先恢复**：上传失败且本地文件仍在时标记为“待补传”，不会重新下载；其他失败任务按全局补传预算逐批恢复。
