...
- 充电专属视频会被准确识别并每7天检查全部启用账号，不再反复启动无权限下载；恢复公开或获得权限后自动继续备份。
- 上传会安全归档同名异大小的远端旧版本，隔离单文件会话异常并分页恢复本地待补传内容，失败不会阻塞其他任务。
- 数据迁移包升级到schema 3，支持轻量/完整ZIP64流式迁移、逐文件校验和原子回滚；临时凭据、日志与登录入口进一步加固。
- SQLite日常查询、失败与冷却读写和启动恢复改为有界索引操作；Debug日志自动轮转，同一BV同画质向多个目标共享一次下载。
- 页面新增版本与GitHub提交入口；内置Compose固定AList `v3.61.0`，并完成B站API、迁移与压缩依赖维护。
...
- **📚 有界状态查询**：收藏夹分页、待确认上传、失败恢复和队列统计直接使用 SQLite 索引；运行时视频与关系缓存各自最多保留 256 条，不再为日常接口反序列化完整状态。
- **🧱 单行状态持久化**：失败记录、用户冷却和下载接口冷却按独立SQLite行增删，孤立待补传记录按游标分页恢复，不再为一条变化重写整组状态。
...
Web面板顶部和登录页会显示应用版本、镜像分支及提交短哈希；点击版本可打开对应GitHub提交，`GitHub ↗`可直接进入项目主页。源码本地运行且没有构建信息时显示`local`。
...
- **失败任务优先恢复**：上传失败且本地文件仍在时标记为“待补传”，不会重新下载；启动时孤立补传记录通过SQLite游标分页写回持久队列，内存仍只领取任务预取上限，单个损坏清单会保留原状态而不形成重试循环。
...
- **画质重调**：无法判断旧文件画质的项目单独列出且默认不选；同一BVID、相同画质/编码/音频/命名档案只下载一次，再分别上传到多个收藏夹目标。切换网页/APP接口或下载账号不会重复生成同一成品；不同画质档案仍保持独立。旧备份DELETE失败时保留新版与本地文件，并显示“旧文件清理重试中”直到远端清理成功。
...
- 当出现 `解析此分P失败` 时，系统会自动执行一次 BBDown 调试探测并将结果保存到：
  - `data/debug/*.log`
- Debug日志使用临时文件原子写入，默认保留14天、最多200个文件且总计不超过256 MiB；任一上限超出时从最旧文件开始清理。
- 可通过环境变量`BFB_DEBUG_LOG_RETENTION_DAYS`、`BFB_DEBUG_LOG_MAX_FILES`和`BFB_DEBUG_LOG_MAX_MIB`调整上限；非法值会回退安全默认值。
