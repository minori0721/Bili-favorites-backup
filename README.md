...
当前dev代码使用SQLite `user_version 8`；已发布`v2.4.6`使用schema 7，`v2.4.5`使用schema 6。schema 7首次升级到schema 8前会在`data/backups`生成一致性数据库备份和SHA256摘要，并在同一事务中构建全局与账号归档投影；旧镜像不能直接打开schema 8数据库，回滚时应恢复对应升级前备份。JSON兼容状态仍为schema 13，迁移包仍为schema 3。
