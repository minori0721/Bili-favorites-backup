...
- **版本与更新（dev新增）**：点击顶部版本标签查看当前构建和正式版发布说明；按需检查、进程内缓存6小时，手动刷新至少间隔1分钟。自动排除FFmpeg等工具发布；说明使用禁用HTML和远端图片的Markdown，正文缺失时可查看对应版本CHANGELOG。dev及本地构建仅展示正式版参考，不自动升级或重启。检查失败不影响归档任务，独立公告和dev镜像更新检测暂未提供。
...
- **自动恢复**：任务日志旁的“待处理”入口集中说明异常和保护措施。远端延迟可见会自动低频复核；至少3次独立观察且持续30分钟仍不可见时才提示人工处理。本地完整文件组可由管理员生成隔离候选；下载耗尽可重试、换账号或暂缓24小时，证据明确的媒体失败可先查看当前画质×编码组合和大小，再严格选择一次新规格。权限、路径和网络错误不会误触发写入。
...
当前`v2.5.4`和dev代码使用SQLite `user_version 11`；`v2.5.0`使用schema 10，`v2.4.6`使用schema 7，`v2.4.5`使用schema 6。schema 11只增加来源类型、手动归档索引和分层封面所需的兼容字段，不重写既有关系；升级前仍会生成一致性数据库备份。`v2.5.1`至`v2.5.4`不再执行数据库结构迁移。旧镜像不能直接打开schema 11数据库，回滚时应恢复对应升级前备份。JSON兼容状态仍为schema 13，迁移包仍为schema 3。

遇到“待补传”时通常不需要直接处理SQLite或进入容器。BFB会先按会话保存的路径和大小只读复核 AList / OpenList：只有存在本次PUT证明的同大小文件才会自动完成；未知同大小、远端异大小或多分P状态不一致时不会冒认成功。已有完整旧归档会继续保留；本地整组文件通过预检后，“待处理”会提供“生成隔离候选”，由管理员确认额外空间和流量后才上传。只有完整旧档与完整候选并存时才需要选择；旧档不完整时自动采用已验证候选，候选不完整时自动保留完整旧档，任何分支都不会自动删除另一份远端文件。
...
存储认证、权限或地址异常时，可在全局设置运行“只读检查存储连接”。它使用当前表单草稿，最多等待15秒，只读取WebDAV端点和归档目录，不保存配置、不写测试文件；检查通过仅证明当前账号可读取目录，写入能力仍应使用隔离的小视频验证。

从`v2.4.3`及更早版本直接更新到`v2.5.4`后需要重新登录一次；之后可在登录页选择固定保持30天。从`v2.4.4`至`v2.5.0`更新会执行相应SQLite迁移，从`v2.5.1`至`v2.5.4`不改变SQLite schema，也不会因本次升级主动撤销现有管理员会话。
...
`v2.5.4`沿用BBDown fork Release `bfb-2.0.4`：除普通下载外提供无签名URL的结构化媒体探测，返回分P、B站档位、编码、尺寸、帧率、时长、码率和估算大小。严格重试会逐分P核对编码与画质，不会静默降级；空间估算只计入最终选中的组合。全局编码偏好仍可在设置中拖动，在线内容使用有界缓存，BVID已有归档封面时不会再次请求图片。

开发分支已接入 [BBDown bfb-2.0.5](https://github.com/minori0721/BBDown/releases/tag/bfb-2.0.5)，修复互动剧情接口并核对完整可达片段清单；普通视频请求流程不变，已有片段按CID续传。此变更尚未作为新的BFB稳定版发布。

“全量扫描并对账”右侧的“归档库”只读取当前`data/bfb.sqlite`和账号配置。全局与账号目录会按BV号合并重复关系，并使用schema 11持久投影完成筛选、搜索和稳定分页；具体收藏夹继续实时读取当前收藏顺序和历史关系。每批读取50项，搜索覆盖归档前标题、UP主和BV号。可播放卡片进入现有播放器，并按当前目录、筛选、排序和基础搜索形成连续队列；同一BV有多份已验证成品时优先选择实际媒体参数更高且更完整的来源。不可播放卡片只展示本地保存的来源与脱敏状态，不会为了补信息临时访问B站。
