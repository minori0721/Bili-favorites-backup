...
- 新增数据迁移包，可导出/导入配置、账号、备份状态、下架记录、日志和封面缓存。
- 已失效视频保留历史标题、UP 主、简介和本地 WebP 封面；本人可见的自投稿失效视频会尝试回填真实信息。
- 优化队列看板和详情列表布局，本地缓存状态改为横条展示，长标题不再挤出状态徽章。
- AList 上传会发送准确文件大小并校验远端结果；不兼容 Emoji 文件名的驱动会自动改用兼容名，单个异常文件不会阻塞其他任务，本地文件仍保留为“待补传”。
- BBDown 固定使用 fork Release `bfb-1.6.3-259a5558.1`，由 aria2 保存字节级进度；容器重启后会复用完整分P、续传未完成轨道，旧版分P单独归档。
- 下载接口可选择“网页接口 / APP接口”；网页接口触发播放风控后会暂停 3 分钟，再用单任务探测自动恢复，不会让失败任务立即循环。
...
- **⚡ 持久化任务队列**：SQLite 原子领取、唯一去重、延迟重试和租约恢复，内存中只保留少量正在执行的任务。
...
- **🧯 上传故障保护**：AList 认证、驱动或网络持续异常时自动暂停新下载；单个确定性失败会隔离补传而不阻塞全局，已下载文件不会因上传失败被删除。
...
- **🧳 数据迁移包**：schema 2 导出包同时包含一致性 SQLite 备份、可读 JSON 快照、下架索引和半尺寸 WebP 封面；旧 schema 1 包仍可导入。
...
- Web 面板的「数据迁移」可导出 zip 迁移包；如果包含账号登录信息，压缩包中会带 B 站 Cookie / token，请当作敏感文件保管。
...
- **BBDown 播放接口**：默认使用网页接口；也可以让所有新任务常驻 APP 接口。网页接口遇到 `v_voucher` 后会全局冷却 3 分钟，只放行原失败任务探测，成功取得播放地址后自动恢复。
