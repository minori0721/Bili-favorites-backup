...
- **自动恢复**：任务日志旁的“待处理”入口集中说明异常和保护措施。远端延迟可见会自动低频复核；本地待补传文件失效且远端确认缺失时会自动重新下载一次，只有异大小冲突、认证错误或数据取舍才要求管理员决定。
...
当前dev代码使用SQLite `user_version 10`；已发布`v2.4.6`使用schema 7，`v2.4.5`使用schema 6。schema 7首次升级到schema 8前会在`data/backups`生成一致性数据库备份和SHA256摘要，并在同一事务中构建全局与账号归档投影；随后schema 9保存持久上传会话与逐文件状态，schema 10为每次上传尝试增加持久化代际并保留旧逐文件明细。旧镜像不能直接打开schema 10数据库，回滚时应恢复对应升级前备份。JSON兼容状态仍为schema 13，迁移包仍为schema 3。
