...
本轮在本地dev分阶段完成前后端模块化，保留既有API、数据schema与首屏修复。没有自动推送、发版或服务器部署。
...
- [x] 核对状态所有者、事务与已有测试耦合。
- [x] 独立前端构建、静态资源清单和开发入口。
- [x] 前端功能迁移，删除唯一过渡脚本。
- [x] 调度规则、事件注册和生命周期拆分；启动恢复、缓存扫描与账号退役的专属状态已迁出。
- [x] 扫描、探测、核验、恢复、画质、清理业务与对应路由拆分；原调度器保留准入、领取、租约、维护屏障和明确依赖装配。
- [x] 架构检查与开发文档已接入；最终代码后端、UI、类型、构建及生产依赖审计通过。
- [x] 最终当前代码 Linux 镜像 HTTP、媒体工具及停止/重启验收：无网络、无宿主挂载，同一隔离数据重启后复验通过，临时容器已清理。FFmpeg 下载阶段复用摘要一致的本地固定版本资产；构建方式与镜像 ID 详见 `DEV_NOTES.md`。

检查清单以实际结果更新，历史测试数字不能作为新快照的验收。
...
运行镜像使用 Tini 作为 PID 1，转发停止信号并回收下载器留下的孤儿进程，Node 继续负责业务停止顺序。构建阶段已改为复用已通过构建的 `node_modules` 并执行 `npm prune --omit=dev`，避免在无 Python 的精简运行镜像中重新编译 `better-sqlite3`。本地 Linux 镜像已完成构建，隔离容器启动到 `Server listening`，并验证 `ffmpeg`、`ffprobe`、`tini` 可执行；媒体测试仍需带上运行镜像中的媒体工具。
...
- 最终验证记录见 `DEV_NOTES.md` 最新检查点。正式发布的远端流水线 digest 验收不属于本地重构，仍按 dev → main → tag 执行。
...
当前记录的是本地 dev 的实施结果；没有提交、推送、发布或部署。画质任务重建及阶段完成转换、恢复标识解析均由独立模块负责。
...
- 修改前端执行严格类型检查和 UI 三视口回归；修改调度/存储执行完整 `npm test`；合并前统一执行构建、架构检查、UI、依赖审计及隔离镜像验收。嵌套测试由入口递归发现，无需手工补文件清单。
