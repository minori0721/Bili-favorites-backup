...
安装依赖、构建（含类型与架构检查）、发布说明校验和镜像隔离启动检查始终执行。镜像先按摘要上传，按该摘要完成启动和层复用检查，再将同一摘要赋给 `dev`、`latest` 或版本标签并逐一核对；启动或层检查失败时不更新部署标签。赋标签本身不是多标签原子操作，若发布中途失败，以各标签的摘要核对结果判断实际状态。跳过测试的运行摘要记录复用的提交 SHA、工作流 SHA 和 dev run 链接及 ID，便于追溯。文档站仍独立构建部署；UI 仍按改动范围在本地验收，不能把此复用规则当成 UI 已自动运行。
...
- 在线收藏夹详情保留 60 秒原始列表缓存；同账号、授权、页和文件夹标题的并发详情请求只执行一次可见性探测与元数据记录，最终页面状态仍逐请求读取。账号 Cookie 替换或维护清缓存后，旧请求不能写入或复用旧列表。
...
旧画质任务迁移按视频及制品归组。清单损坏的组保留原任务并进入人工等待，正常组继续迁移；仍有未处理组时不写入完成标记。任务替换与暂停共同处于 SQLite 事务，归属变化或数据库写入失败中止并回滚。

每次启动检查已验证归档的本地清单，不受队列初始化标记影响。损坏清单不降低远端验证状态，也不盲目重下载；日志提示历史文件身份需要人工处理。依赖该证据的历史上传、画质恢复和清理停止，其他任务继续。保留目录统计普通文件实际字节，跳过符号链接；读取失败不伪装成零占用。

清单错误携带字段位置，例如 `outputs[0].size`，不输出原文。内部直接使用已解码字段，仍保留文件大小、路径、租约、代次和远端证明复核。

B站列表按接口验证身份及分页。坏条目使本页失败，不能当空页或标记扫描完成；合法空列表和非视频历史记录仍受支持。兼容依据包括社区采集的[历史记录样本](https://github.com/pskdje/bilibili-API-collect/blob/main/docs/historytoview/history.md)、[稍后再看接口](https://github.com/pskdje/bilibili-API-collect/blob/main/docs/historytoview/toview.md)及[收藏合集空列表格式](https://lxb007981.github.io/bilibili-API-collect/user/space.html)。这些是历史协议样本而非官方稳定性保证；合集 `list: null` 仅在 `count: 0` 时允许，缺失分页不能推断为结束。测试数据不能单独证明新兼容格式。
