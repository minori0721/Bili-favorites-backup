...
- 当前dev代码会把schema 7数据库升级到schema 8并构建全局与账号归档投影，再升级到schema 9保存持久上传会话与逐文件状态，最后升级到schema 10增加上传代际并保留旧尝试明细。schema 7到8在1万个唯一BV、5万条关系的测试库本地约需21秒；schema 9到10只重建上传会话明细表，实际时间取决于上传尝试记录规模，启动期间服务尚不可用。回滚到已发布`v2.4.6`或其他只支持schema 9及更早版本的镜像时，先恢复`data/backups`中的对应升级前SQLite备份，不能让旧镜像直接打开schema 10数据库。
...
4. schema 8投影、schema 9上传会话和schema 10上传代际升级日志均完成，归档库全局、账号和收藏夹目录均可正常翻页，队列可恢复持久上传与远端确认。
5. 队列没有同一BV的立即失败循环。
