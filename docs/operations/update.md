...
- `v2.5.1`会把schema 10数据库升级到schema 11，增加手动来源类型与相关索引；`v2.5.2`沿用schema 11，不再执行数据库结构迁移，不批量探测在线内容、不重写既有关系。升级前会在`data/backups`生成一致性备份，启动期间服务暂不可用；回滚到`v2.5.0`或更早镜像时，先恢复升级前SQLite备份，不能让旧镜像直接打开schema 11数据库。
