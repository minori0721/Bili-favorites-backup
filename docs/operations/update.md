...
- `v2.5.0`会把schema 7数据库升级到schema 8并构建全局与账号归档投影，再升级到schema 9保存持久上传会话与逐文件状态，最后升级到schema 10增加上传代际并保留旧尝试明细。schema 7到8在1万个唯一BV、5万条关系的测试库本地约需21秒；schema 9到10只重建上传会话明细表，实际时间取决于上传尝试记录规模，启动期间服务尚不可用。回滚到`v2.4.6`或更早镜像时，先恢复`data/backups`中的升级前SQLite备份，不能让旧镜像直接打开schema 10数据库。
