...
- 标签必须为`vX.Y.Z`，并与package版本及CHANGELOG章节一致。CHANGELOG应记录主要变化、升级注意、数据兼容与已知问题，从v2.5.6开始，发布正文提取标签对应章节的“发布摘要”与“升级与回滚”，保留重要限制；历史版本兼容完整章节，不取dev“未发布”内容。完整CHANGELOG与文档站继续维护详细说明。
- `Docker Publish`中新增的Release任务只在正式标签镜像成功后运行，单独授予`contents: write`；重跑保留已有Release，历史版本不会把Latest退回。此流程从v2.5.5开始生效，不追溯执行旧工作流。
- 推送main前预览发布正文并验证最终提交；dev验证通过后快进main，等待main镜像与文档完成，再创建同提交的正式标签。分别核验镜像revision、Release与文档站；服务器升级单独执行。
