...
`v2.5.1`固定BBDown fork `bfb-2.0.4`，会为每个账号保存独立的APP设备标识，并提供仅返回媒体描述的结构化探测。探测可以在待处理重试或在线内容手动归档前按需执行；它不返回签名播放地址。普通下载仍以ffprobe实际文件为最终证明，旧版BBDown缺少探测协议时只禁用精确探测，不影响普通下载。
