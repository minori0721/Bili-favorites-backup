...
未发布的dev已固定BBDown fork `bfb-2.0.1`，会为每个账号保存独立的APP设备标识，并使用PlayerUnite读取普通UGC的AVC、HEVC和AV1流。APP最高仅480P或低于明确请求档位时，BBDown只在WEB视频更高时合并WEB视频，APP音频保持不变；WEB比较失败不会让已经取得的APP结果失败。当前`v2.4.5`稳定镜像仍使用`bfb-2.0.0`。
