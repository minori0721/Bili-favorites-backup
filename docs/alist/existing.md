...
远端 WebDAV 账号必须能在目标目录执行：列出、创建目录、PUT 上传、DELETE 删除和读取文件属性。普通新归档不要求 MOVE；历史归档、画质替换和批量重命名会优先使用 MOVE，只有明确不支持 MOVE 且 COPY 已通过目标大小校验时才使用 COPY+DELETE。因此需要同时允许 COPY，缺少 DELETE 会让旧文件清理持续重试但不会删除本地新版，缺少 COPY/MOVE 时相应替换操作会安全暂停。OpenList 还需要为该账号授予对应挂载的 WebDAV 管理权限。
