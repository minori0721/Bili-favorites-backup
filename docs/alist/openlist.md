...
4. GET/Range 播放或下载文件。
5. DELETE 删除已确认的文件。
6. MOVE 用于历史归档、重命名和画质替换的优先路径；如果明确不支持 MOVE，BFB 会在 COPY 探测通过并完成目标大小校验后使用 COPY+DELETE。
...
| 旧命名检查遇到撇号或百分号文件 | OpenList的WebDAV目录清单可能返回带反斜杠的访问写法 | BFB会在本地证明唯一且大小一致时重新解析后处理；存在多个匹配或证明不足时跳过，不会放宽路径校验或猜测文件身份 |
