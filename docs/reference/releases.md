...
- 新下载同时保存BBDown实际选中的B站档位和ffprobe媒体参数；播放器不展示请求配置，旧归档缺少B站档位时直接省略。
- 播放器显示本次真实直连/代理状态，并可通过独立配置的AList网页地址打开对应文件。
- 非标准分辨率按真实短边显示，播放器区分网络、解码和HEVC兼容错误；解码失败不再无意义地切换BFB代理。
- SQLite升级到`user_version 6`；JSON兼容状态仍为schema 13，迁移包仍为schema 3。正式版本号尚未调整。
