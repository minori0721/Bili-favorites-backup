...
export function createQueueBoardProjection(deps: { metadata: StateManager['getVideoMetaBatch'] }) {
  ...
  function mapPersistentJobForBoard(job: PersistentJobRecord): QueueBoardItem | null {
    ...
    return mapQueueBoardTask({
      ...
      userId: job.userId,
      mediaId: job.mediaId,
      videoTitle: String(payload.videoTitle || job.bvid || ""),
      upperName: String(payload.upperName || "" || ""),
      cover: String(payload.cover || "" || ""),
      coverLocalPath: typeof payload.coverLocalPath === "string" ? payload.coverLocalPath : undefined,
      folderTitle: String(payload.folderTitle || payload.primaryFolderTitle || "" || ""),
      remotePath: String(payload.remotePath || payload.remoteFile || "" || ""),
      detail,
      status: job.status,
