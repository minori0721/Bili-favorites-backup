...
function object(value: unknown) {
  if (!isRecord(value)) throw new Error('任务中心状态格式错误');
  return value;
}
function text(value: unknown) {
  if (value == null) return '';
  if (typeof value !== 'string') throw new Error('任务中心状态说明格式错误');
  return value;
}
function count(value: unknown) {
  if (value == null) return 0;
  if (typeof value !== 'number' || !Number.isFinite(value)) throw new Error('任务中心状态计数格式错误');
  return value;
}
function time(value: unknown): string | number | undefined {
  if (value == null) return undefined;
  if (typeof value === 'string' || (typeof value === 'number' && Number.isFinite(value))) return value;
  throw new Error('任务中心状态时间格式错误');
}
function flag(value: unknown) {
  if (value == null) return false;
  if (typeof value !== 'boolean') throw new Error('任务中心状态标记格式错误');
  return value;
}
export function parseQueueStatus(value: unknown) {
  ...
  if (!Array.isArray(actions) || !actions.every((item):item is string=>typeof item==='string')) throw new Error('调度操作列表格式错误');
  return {
